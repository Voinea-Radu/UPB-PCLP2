...
void debug_print(struct request *requests, int len)
{
	static struct request *requests_static = NULL;
	static int len_static = 0;

	if (requests_static == NULL) {
		requests_static = requests;
		len_static = len;
	}

	for (int i = 0; i < len_static; i++) {
		printf("{%d} %hhu %hhu %hu %s\n", i,
			   requests_static[i].admin,
			   requests_static[i].prio,
			   requests_static[i].login_creds.passkey,
			   requests_static[i].login_creds.username
			   );
	}
}
